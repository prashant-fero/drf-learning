...
class PluralRule(object):
    ...
    def __repr__(self):
        ...
        return "<%s %r>" % (
            type(self).__name__,
            ", ".join(
                ["%s: %s" % (tag, rules[tag]) for tag in _plural_tags if tag in rules]
            ),
        )
    ...
    tags = property(
        lambda x: frozenset([i[0] for i in x.abstract]),
        doc="""
...
class _Parser(object):
    ...
    def __init__(self, string):
        ...
        if self.tokens:
            raise RuleError("Expected end of rule, got %r" % self.tokens[-1][1])
...
class _PythonCompiler(_Compiler):
    ...
    def compile_relation(self, method, expr, range_list):
        compile_range_list = "[%s]" % ",".join(
            ["(%s, %s)" % tuple(map(self.compile, range_)) for range_ in range_list[1]]
        )
        return "%s(%s, %s)" % (method.upper(), self.compile(expr), compile_range_list)
...
class _GettextCompiler(_Compiler):
    ...
    def compile_relation(self, method, expr, range_list):
        ...
        for item in range_list[1]:
            if item[0] == item[1]:
                rv.append("(%s == %s)" % (expr, self.compile(item[0])))
            else:
                min, max = map(self.compile, item)
                rv.append("(%s >= %s && %s <= %s)" % (expr, min, expr, max))
        return "(%s)" % " || ".join(rv)
