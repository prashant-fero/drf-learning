...
class LazyProxy(object):
    ...
    def __copy__(self):
        return LazyProxy(
            self._func, enable_cache=self._is_cache_enabled, *self._args, **self._kwargs
        )
...
class NullTranslations(gettext.NullTranslations, object):
    ...
    def lpgettext(self, context, message):
        ...
        warnings.warn(
            "lpgettext() is deprecated, use pgettext() instead", DeprecationWarning, 2
        )
    ...
    def lnpgettext(self, context, singular, plural, num):
        ...
        warnings.warn(
            "lnpgettext() is deprecated, use npgettext() instead", DeprecationWarning, 2
        )
        ctxt_msg_id = self.CONTEXT_ENCODING % (context, singular)
        try:
            tmsg = self._catalog[(ctxt_msg_id, self.plural(num))]
